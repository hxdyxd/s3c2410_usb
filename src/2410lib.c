...
void Port_Init(void)
{
    ...
    rGPGCON = 0xff81ffba;	//GPG10 input,GPG9 input
...
void Uart2_Test( void )
{
	...
	while( (m=Uart_GetKey()) != ESC_KEY )
	{
	   	if( m != 0 )  
	   	{
	   		puts( "'ESC' key return to UART1, The input of you is " ) ;
	   		putch( m ) ;		puts( "\r\n" ) ;
	   		n = 1 ;
	   	}
	   	else  
	   	{
	   		if( n==0 )
	   		{
		   		puts( "Now is UART2 output, do you see?  Any key to display, 'ESC' key return to UART1 \r\n" ) ;
		   		n = 0 ;
		   		Delay( 6000 ) ;
	   		}
	   	}
